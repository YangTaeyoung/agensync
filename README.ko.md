...
**감지된 도구** → `From` → `To (다중 선택)` → `카테고리` → `옵션: 덮어쓰기? 재귀?` → **플랜 미리보기 + 손실 리포트** → **적용 + 진행 내역**

</div>

대화형 모드는 현재 프로젝트에서 감지된 도구를 먼저 보여주고, **From**/**To**를 고른 뒤, **덮어쓰기**(기존 파일; `.bak` 백업은 항상 유지)와 **하위 디렉터리 재귀**(모노레포)를 토글하고, 무엇이 바뀔지 정확히 미리보기로 확인한 다음 적용합니다. `↑/↓` 이동 · `space` 토글 · `enter` 다음 · `esc` 뒤로 · `q` 종료.
