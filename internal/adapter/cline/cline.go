...
// ExportProjectState: cline config is profile-global, not path-keyed.
func (Cline) ExportProjectState(ir.Context) (ir.ProjectState, error) {
...
func (a Cline) PlanImport(b ir.AgentConfigBundle, ctx ir.Context, opts adapter.ImportOptions) ir.WritePlan {
	...
	// are merged into the single global file; isolation is lost (warn).
	if opts.Wants("mcp") && len(b.McpServers) > 0 && ctx.HomeDir != "" {
